...
public class MainController extends VBox implements Initializable{
    ...
    protected void runC45() throws Exception{
        final int historyIndex = historyList.size() - 1;
        final Instances b = new Instances(base);

        try {
            J48 c45 = new J48();
            c45.setConfidenceFactor(Math.round(sliderConfidenceFactorC45.getValue())/100.0f);
            c45.setMinNumObj((int) Math.round(sliderMinLeaftC45.getValue()));
    ...
    protected void runRuleBased() throws Exception {
        ...
        try {
            OneR RB = new OneR();
            RB.setMinBucketSize((int) Math.round(sliderMinBucketSizeRB.getValue()));
    ...
    protected void runNeuralNetwork() throws Exception {
        ...
        try {
            ...
            NN.setLearningRate(Math.round(sliderLearningRateNN.getValue())/100.0);
            NN.setMomentum(Math.round(sliderMomentumNN.getValue())/100.0);
            NN.setValidationSetSize((int) Math.round(sliderMomentumNN.getValue()));
    ...
    protected void runDeepLearning() throws Exception {
        final int historyIndex = historyList.size() - 1;
        final Instances b = new Instances(base);
        final int nAttr = b.numAttributes() - 1;
        final double learningRate = Math.round(sliderLearningRateDL.getValue()) / 100.0;
        final int[] kernelSize = new int[]{1,(int) Math.round(sliderKernelDL.getValue())};
        final int[] strideSize = new int[]{(int) Math.round(sliderStrideDL.getValue()), 1};
        ...
        try {
            ...
            for(int i = 0; i < hiddenLayersParams.length; i++) {
                ...
                convLayer.setKernelSize(kernelSize);
